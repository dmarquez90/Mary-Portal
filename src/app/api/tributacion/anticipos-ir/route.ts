...
export async function POST(req: NextRequest) {
  ...
  // Calcular ingresos brutos del mes automáticamente si no se especifican
  ...
  const tasa = 0.01
  const monto_anticipo = Number((ingresos * tasa).toFixed(2))
  const retenciones = body.retenciones_recibidas ?? 0
  const monto_a_pagar = Math.max(0, monto_anticipo - retenciones)
  ...
  const { data, error } = await supabase
    ...
    .upsert({
      ...
      fecha_declaracion: body.fecha_declaracion,
      fecha_pago: body.fecha_pago,
      numero_boleta: body.numero_boleta
