...
export default function CxCPage() {
  ...
  const [filtro, setFiltro] = useState<string>('vigente')
  ...
  async function registrarAbono() {
    ...
    if (r.ok) {
      setModalAbono(null)
      setAbono({ monto: '', fecha: new Date().toISOString().split('T')[0], forma_pago: 'transferencia', referencia: '', notas: '' })
  ...
  const totalPendiente = saldos.filter(s => s.estado_cobro !== 'pagada').reduce((s, c) => s + c.saldo_pendiente, 0)
  const totalVencido = saldos.filter(s => s.estado_cobro === 'vencida').reduce((s, c) => s + c.saldo_pendiente, 0)

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Cuentas por Cobrar (CxC)</h1>
        <p className="text-sm text-gray-500 mt-1">Cartera de crédito a clientes — Antigüedad de saldo — Pagos parciales</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gray-50 rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-gray-700">{saldos.filter(s => s.estado_cobro !== 'pagada').length}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {['vigente','vencida','pagada','todos'].map(e => (
          <button key={e} onClick={() => { setFiltro(e); fetchSaldos(empresaId, e) }}
            className={`px-3 py-1 text-xs rounded-full border ${filtro === e ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600'}`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-400">
          ...
          <p>No hay saldos pendientes en esta categoría</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                {['Factura','Cliente','Emisión','Vencimiento','Original','Abonado','Saldo','Estado','Acción'].map(h => (
            ...
            <tbody className="divide-y">
              {saldos.map(s => (
                <tr key={s.factura_id} className={`hover:bg-gray-50 ${s.estado_cobro === 'vencida' ? 'bg-red-50' : ''}`}>
                  <td className="px-3 py-3 font-mono text-xs font-medium">{s.numero_factura}</td>
                  <td className="px-3 py-3">
                    <p className="font-medium">{s.cliente?.nombre ?? '—'}</p>
                    {s.cliente?.telefono && <p className="text-xs text-gray-400">{s.cliente.telefono}</p>}
                  </td>
                  <td className="px-3 py-3 text-xs">{s.fecha_emision}</td>
                  <td className={`px-3 py-3 text-xs ${s.estado_cobro === 'vencida' ? 'text-red-600 font-bold' : ''}`}>
                    {s.fecha_vencimiento ?? '—'}
                  ...
                  <td className="px-3 py-3 font-bold text-blue-700">{fmt(s.saldo_pendiente)}</td>
                  <td className="px-3 py-3">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="p-6 space-y-4">
              ...
              <div className="bg-blue-50 rounded-lg p-3 text-sm">
                <p><strong>{modalAbono.numero_factura}</strong> — {modalAbono.cliente?.nombre}</p>
                <p>Saldo pendiente: <strong className="text-blue-700">{fmt(modalAbono.saldo_pendiente)}</strong></p>
              </div>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium mb-1">Monto del Abono (C$) *</label>
                ...
                <div>
                  ...
                  <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="# transferencia, cheque, etc."
