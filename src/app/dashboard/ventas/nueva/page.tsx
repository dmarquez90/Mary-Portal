...
export default function NuevaFacturaPage() {
  ...
  // ── Moneda de la cuenta de cobro elegida ──────────────────────
  // El total de la factura siempre está en córdobas (NIO); si la
  // cuenta elegida es en USD, el trigger fn_contabilizar_factura
  // convierte el lado operativo (movimientos_caja/transacciones_banco)
  // con esta tasa. Se precarga la tasa vigente pero es editable.
  ...
  async function handleSave(estado: "borrador" | "emitida") {
    ...
    if (lineasValidas.length === 0) {
      toast.error("Agrega al menos un producto o servicio con descripción.");
      return;
    }

    // ── Validar stock antes de emitir ─────────────────────
    // El trigger BEFORE INSERT en detalle_facturas también lo valida,
    // pero esta validación frontend da mejor UX con mensaje claro.
    if (estado === "emitida") {
    ...
    // ── Resolver cuenta banco/caja para pasar al trigger ──
    // El trigger fn_contabilizar_factura usará estos IDs para registrar
    // en la cuenta correcta (no la primera por created_at)
    ...
    // El trigger AFTER INSERT descuenta stock_actual y registra movimiento.
    // El trigger fn_contabilizar_factura genera el asiento y registra en caja/banco.
    // El frontend NO debe hacer nada de eso — los triggers lo manejan todo.
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex gap-2">
                  {!usarClienteLibre ? (
                    ...
                    <input type="text" className="input flex-1" placeholder="Nombre del cliente"
                      value={clienteLibre} onChange={e => setClienteLibre(e.target.value)} />
                  )}
                  <button type="button" onClick={() => { setUsarClienteLibre(v => !v); setClienteId(""); setClienteLibre(""); }}
              ...
              {/* El total de la factura siempre está en córdobas (NIO); si la
                  cuenta elegida es en USD, el sistema convierte el cobro a
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {lineas.map((linea, idx) => {
                ...
                const stockBajo = prod && linea.producto_id && linea.cantidad > prod.stock_actual;
                return (
                  <div key={idx} className={`grid grid-cols-12 gap-2 items-start border rounded-lg p-3 ${stockBajo ? "border-red-300 bg-red-50" : "border-slate-100 bg-slate-50"}`}>
                    <div className="col-span-12 md:col-span-4">
                      ...
                      {/* Aviso de stock insuficiente inline */}
                      {stockBajo && (
                    ...
                    <div className="col-span-12 md:col-span-3">
                      ...
                      <input type="text" className="input text-sm" value={linea.descripcion}
                    ...
                    <div className="col-span-4 md:col-span-1">
                      ...
                      <input type="number" className="input text-sm" min="0" step="0.01" value={linea.cantidad}
                        onChange={e => updateLinea(idx, "cantidad", parseFloat(e.target.value) || 0)} />
                    </div>
                    <div className="col-span-4 md:col-span-2">
                      <label className="label text-xs">Precio</label>
                      <input type="number" className="input text-sm" min="0" step="0.01" value={linea.precio_unitario}
                        onChange={e => updateLinea(idx, "precio_unitario", parseFloat(e.target.value) || 0)} />
                    </div>
                    <div className="col-span-4 md:col-span-1">
                      <label className="label text-xs">Desc.%</label>
                      <input type="number" className="input text-sm" min="0" max="100" value={linea.descuento_pct}
                    ...
                    <div className="col-span-12 md:col-span-1 flex items-center justify-between md:justify-center gap-2 pt-1">
                      <label className="flex items-center gap-1 text-xs text-slate-600 cursor-pointer">
                        <input type="checkbox" className="w-3.5 h-3.5" checked={linea.aplica_iva}
                          onChange={e => updateLinea(idx, "aplica_iva", e.target.checked)} />
                        IVA
                      </label>
                      <button type="button" onClick={() => { if (lineas.length > 1) setLineas(prev => prev.filter((_, i) => i !== idx)); }}
        ...
        <div>
          <div className="card sticky top-6">
            ...
            {/* Aviso de stock insuficiente en alguna línea */}
            {lineas.some(l => {
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4 flex gap-2 text-sm text-red-700">
                ...
                <span>Hay productos con stock insuficiente. No podrás emitir la factura.</span>
              </div>
            )}
