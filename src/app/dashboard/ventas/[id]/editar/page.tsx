...
export default function EditarFacturaPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (fac) {
        ...
        setClienteLibre(fac.cliente_nombre ?? "");
        setUsarClienteLibre(!fac.cliente_id && !!fac.cliente_nombre);
        setFechaEmision(fac.fecha_emision ?? new Date().toISOString().split("T")[0]);
  ...
  async function handleSave(estado: "borrador" | "emitida") {
    ...
    // ── Consecutivo ───────────────────────────────────────
    // ── MODO EDICIÓN: actualizar factura borrador ─────────
