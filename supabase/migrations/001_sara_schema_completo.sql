-- ============================================================
-- SARA ERP – Schema completo
-- Ley 822 LCT / DGI VET – Nicaragua
-- Todas las tablas usan CREATE TABLE IF NOT EXISTS (seguro re-ejecutar)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS empresas_persona_natural (
  ...
  tipo_empresa        TEXT NOT NULL DEFAULT 'persona_natural'
                      CHECK (tipo_empresa IN ('persona_natural', 'cuota_fija')),
...
CREATE TABLE IF NOT EXISTS proveedores (
  ...
  tipo_persona  TEXT NOT NULL DEFAULT 'juridica'
                CHECK (tipo_persona IN ('natural', 'juridica', 'cuota_fija', 'gran_contribuyente')),
  activo        BOOLEAN NOT NULL DEFAULT TRUE,
  created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at    TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- ============================================================
-- BLOQUE 5: FACTURACIÓN DE VENTAS
-- ============================================================
...
CREATE TABLE IF NOT EXISTS facturas (
  ...
  tipo_pago          TEXT NOT NULL DEFAULT 'contado'
                     CHECK (tipo_pago IN ('contado', 'credito', 'transferencia', 'cheque', 'tarjeta')),
  estado             TEXT NOT NULL DEFAULT 'borrador'
                     CHECK (estado IN ('borrador', 'emitida', 'parcial', 'pagada', 'anulada')),
...
CREATE TABLE IF NOT EXISTS compras (
  ...
  tipo_pago         TEXT NOT NULL DEFAULT 'contado'
                    CHECK (tipo_pago IN ('contado', 'credito', 'transferencia', 'cheque', 'tarjeta')),
  estado            TEXT NOT NULL DEFAULT 'borrador'
                    CHECK (estado IN ('borrador', 'recibida', 'parcial', 'pagada', 'anulada')),
  subtotal          NUMERIC(15,2) NOT NULL DEFAULT 0,
  iva_total         NUMERIC(15,2) NOT NULL DEFAULT 0,
  total             NUMERIC(15,2) NOT NULL DEFAULT 0,
  notas             TEXT,
...
-- FK lotes → compras (se agrega después de crear ambas tablas)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_lotes_compra'
  ) THEN
...
-- ============================================================
-- ÍNDICES (seguros: solo crea si la columna existe)
-- ============================================================
...
BEGIN
  CALL safe_index('idx_facturas_empresa',      'facturas',             'empresa_id');
  CALL safe_index('idx_facturas_fecha',        'facturas',             'fecha_emision');
  CALL safe_index('idx_facturas_cliente',      'facturas',             'cliente_id');
  CALL safe_index('idx_compras_empresa',       'compras',              'empresa_id');
  CALL safe_index('idx_compras_fecha',         'compras',              'fecha_compra');
  CALL safe_index('idx_compras_proveedor',     'compras',              'proveedor_id');
  CALL safe_index('idx_detalle_fact',          'detalle_facturas',     'factura_id');
  CALL safe_index('idx_detalle_comp',          'detalle_compras',      'compra_id');
  CALL safe_index('idx_lotes_producto',        'lotes_inventario',     'producto_id');
  CALL safe_index('idx_lotes_fecha',           'lotes_inventario',     'fecha_entrada');
  CALL safe_index('idx_movinv_producto',       'movimientos_inventario','producto_id');
  ...
  CALL safe_index('idx_consecutivos_empresa',  'consecutivos',         'empresa_id');
  CALL safe_index('idx_clientes_empresa',      'clientes',             'empresa_id');
  CALL safe_index('idx_proveedores_empresa',   'proveedores',          'empresa_id');
  CALL safe_index('idx_productos_empresa',     'productos',            'empresa_id');
...
DO $$ BEGIN
  ...
  -- Detalles (acceso via padre)
  DROP POLICY IF EXISTS "via_factura" ON detalle_facturas;
  CREATE POLICY "via_factura" ON detalle_facturas
    FOR ALL USING (
      factura_id IN (
        SELECT id FROM facturas WHERE empresa_id = ANY(get_empresa_ids())
      )
    );

  DROP POLICY IF EXISTS "via_compra" ON detalle_compras;
  CREATE POLICY "via_compra" ON detalle_compras
    FOR ALL USING (
      compra_id IN (
        SELECT id FROM compras WHERE empresa_id = ANY(get_empresa_ids())
      )
    );
