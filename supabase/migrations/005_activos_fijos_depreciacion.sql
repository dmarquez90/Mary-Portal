...
CREATE TABLE IF NOT EXISTS activos_fijos (
  ...
  categoria             TEXT NOT NULL
                        CHECK (categoria IN ('edificio','equipo_produccion','vehiculo','mobiliario','equipo_tic','otro')),
  tasa_depreciacion_anual NUMERIC(5,4) NOT NULL,
  ...
  fecha_inicio_dep      DATE NOT NULL,
  vida_util_anios       NUMERIC(5,2) NOT NULL,
  ...
  estado                TEXT NOT NULL DEFAULT 'activo' CHECK (estado IN ('activo','depreciado','vendido','dado_de_baja')),
  fecha_baja            DATE,
  motivo_baja           TEXT,
...
CREATE TABLE IF NOT EXISTS depreciaciones (
  ...
  dep_acumulada_ant   NUMERIC(15,2) NOT NULL DEFAULT 0,
  dep_acumulada_post  NUMERIC(15,2) NOT NULL DEFAULT 0,
  valor_en_libros     NUMERIC(15,2) NOT NULL,
  estado              TEXT NOT NULL DEFAULT 'calculada' CHECK (estado IN ('calculada','contabilizada','anulada')),
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS activos_intangibles (
  ...
  nombre                  TEXT NOT NULL,
  descripcion             TEXT,
  tipo                    TEXT NOT NULL CHECK (tipo IN ('patente','marca','gastos_organizacion','gastos_preoperativos','licencia_software','otro')),
  costo_adquisicion       NUMERIC(15,2) NOT NULL,
  valor_en_libros         NUMERIC(15,2) NOT NULL,
  fecha_adquisicion       DATE NOT NULL,
  fecha_inicio_amort      DATE NOT NULL,
  vida_util_anios         NUMERIC(5,2) NOT NULL DEFAULT 3,
  ...
  estado                  TEXT NOT NULL DEFAULT 'activo' CHECK (estado IN ('activo','amortizado','dado_de_baja')),
  created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (empresa_id, codigo)
);
...
DO $$ BEGIN
  ...
  CREATE POLICY "own_empresa" ON activos_fijos FOR ALL USING (empresa_id = ANY(get_empresa_ids()));
  DROP POLICY IF EXISTS "own_empresa" ON depreciaciones;
  CREATE POLICY "own_empresa" ON depreciaciones FOR ALL USING (empresa_id = ANY(get_empresa_ids()));
  DROP POLICY IF EXISTS "own_empresa" ON activos_intangibles;
  CREATE POLICY "own_empresa" ON activos_intangibles FOR ALL USING (empresa_id = ANY(get_empresa_ids()));
  DROP POLICY IF EXISTS "own_empresa" ON amortizaciones;
  CREATE POLICY "own_empresa" ON amortizaciones FOR ALL USING (empresa_id = ANY(get_empresa_ids()));
END$$;
...
CREATE OR REPLACE FUNCTION calcular_cuota_depreciacion(p_costo NUMERIC, p_residual NUMERIC, p_vida_meses INT)
RETURNS NUMERIC AS $$ SELECT ROUND((p_costo - p_residual) / NULLIF(p_vida_meses, 0), 2); $$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION tasa_depreciacion_lct(p_categoria TEXT)
RETURNS NUMERIC AS $$
  SELECT CASE p_categoria
    WHEN 'edificio' THEN 0.05 WHEN 'equipo_produccion' THEN 0.20
    WHEN 'vehiculo' THEN 0.20 WHEN 'mobiliario' THEN 0.20
    WHEN 'equipo_tic' THEN 0.50 ELSE 0.10
  END;
