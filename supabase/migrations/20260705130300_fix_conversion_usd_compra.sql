-- ============================================================
-- Multi-moneda (4/4a): fn_contabilizar_compra convierte el lado
-- operativo (movimientos_caja/transacciones_banco/cheques) cuando
-- la cuenta caja/banco resuelta está en USD. Mismo patrón que la
-- migración hermana para facturas.
-- ============================================================
...
BEGIN
  ...
  -- El asiento contable siempre se postea en córdobas, sin importar la
  -- moneda de la cuenta que finalmente pague.
  ...
  -- ── Movimiento operativo: convierte a la moneda real de la cuenta destino ──
  ...
  ELSIF NEW.tipo_pago IN ('transferencia','cheque','tarjeta') THEN
    ...
    IF v_cuenta_banco_id IS NOT NULL THEN
      ...
      -- ── Si es cheque, registrar en tabla cheques (monto ya convertido) ──
